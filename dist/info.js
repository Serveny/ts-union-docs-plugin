...
exports.TypeInfoFactory = exports.UnionParameterInfo = exports.TypeInfo = void 0;
...
exports.UnionParameterInfo = UnionParameterInfo;
class TypeInfoFactory {
    ...
    findNodeAtPos(srcFile, pos) {
        const find = (node) => pos >= node.getStart() && pos < node.getEnd()
    ...
    getCallExpression(node) {
        if (this.ts.isCallExpression(node))
            return node;
        while (node && !this.ts.isCallExpression(node))
            node = node.parent;
        return node;
    }
    ...
    collectUnionMemberNodes(node) {
        ...
        if (ts.isTypeLiteralNode(node)) {
            return node.members
                .map((m) => m.type ? this.collectUnionMemberNodes(m.type) : [])
                .flat();
        }
        if (ts.isMappedTypeNode(node)) {
            const results = [];
            if (node.typeParameter.constraint)
                results.push(...this.collectUnionMemberNodes(node.typeParameter.constraint));
            if (node.type)
                results.push(...this.collectUnionMemberNodes(node.type));
            return results;
        }
        ...
        if (ts.isTypeOperatorNode(node)) {
            if (node.operator === ts.SyntaxKind.KeyOfKeyword) {
                ...
                return type.getProperties().map((p) => {
                    ...
                    const node = ts.factory.createLiteralTypeNode(ts.factory.createStringLiteral(p.getName()));
...
exports.TypeInfoFactory = TypeInfoFactory;
