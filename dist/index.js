...
var SupportedType = /* @__PURE__ */ function(SupportedType) {
	...
	SupportedType[SupportedType["Variable"] = 1] = "Variable";
...
var TypeInfoFactory = class {
	...
	getTypeInfo(fileName, position) {
		...
		if (this.typeInfoCache.has(cacheKey)) return this.typeInfoCache.get(cacheKey) ?? null;
	...
	getCompletionInfo(fileName, position) {
		const cacheKey = `${fileName}:${position}`;
		if (this.completionInfoCache.has(cacheKey)) return this.completionInfoCache.get(cacheKey) ?? null;
		const context = this.getCompletionContext(fileName, position);
		if (!context) {
			this.completionInfoCache.set(cacheKey, null);
			return null;
		}
		const unionMemberNodes = this.collectUnionMemberNodes(context.typeNode, void 0, context.typeArgMap);
	...
	getDeprecatedUsageInfos(fileName) {
		...
		const visit = (node) => {
			if (isDeprecatedUsageNode(this.ts, node)) for (const info of this.getTypeInfoForExpression(node)) {
	...
	getUnionInfoForArgument(expr) {
		...
		const paramSymbol = this.checker.getResolvedSignature(callLike)?.getParameters()[argIndex];
	...
	getCompletionContext(fileName, position) {
		...
		const typeNode = parameterContext?.typeNode ?? this.resolveTypeNode(node, contextualType);
	...
	getParameterTypeContext(node) {
		...
		if (argIndex < 0) return null;
		const paramDecl = (signature?.getParameters()[argIndex])?.getDeclarations()?.[0];
		...
		const resolvedTypeNode = this.resolveMappedTypeNode(paramDecl.type, typeArgMap) ?? paramDecl.type;
		return {
			typeNode: this.isDirectTypeParameterReference(paramDecl.type) && this.ts.isLiteralTypeNode(resolvedTypeNode) ? this.getTypeParameterConstraintNode(paramDecl.type) ?? paramDecl.type : paramDecl.type,
			typeArgMap,
			templateFilterType: resolvedTypeNode === paramDecl.type ? void 0 : this.checker.getTypeAtLocation(resolvedTypeNode)
		};
	}
	getTypeParameterConstraintNode(typeNode) {
		const decl = this.getTypeReferenceSymbol(typeNode)?.declarations?.[0];
		return decl && this.ts.isTypeParameterDeclaration(decl) ? decl.constraint ?? null : null;
	}
	...
	getTypeNodeFromInitializer(node) {
		...
		if (this.ts.isVariableDeclaration(parent) || this.ts.isPropertyDeclaration(parent)) return parent.initializer === node ? parent.type ?? null : null;
		return null;
	}
	filterRegexMembers(members, contextualType) {
		return members.filter((memberNode) => {
			...
			if (this.ts.isTemplateLiteralTypeNode(memberNode)) return (contextualType.getFlags() & this.ts.TypeFlags.StringLike) !== 0;
	...
	findCallLikeExpression(node) {
		...
		while (current && !this.ts.isSourceFile(current)) {
			if (this.ts.isCallExpression(current) || this.ts.isNewExpression(current)) return current;
	...
	getExpressionName(expr) {
		...
		if (parent && (this.ts.isVariableDeclaration(parent) || this.ts.isPropertyDeclaration(parent) || this.ts.isParameter(parent)) && this.ts.isIdentifier(parent.name)) return parent.name.text;
		return "value";
	}
	...
	addSignatureTypeParameterMappings(map, callLike, signatureDecl) {
		...
		for (let i = 0; i < typeParams.length; i++) {
			...
			const inferredArg = this.inferTypeArgumentFromParameters(callLike, signatureDecl, symbol);
			const normalizedArg = this.normalizeTypeArgument(typeParam, inferredArg);
			if (normalizedArg) map.set(symbol, normalizedArg);
		}
	}
	addEnclosingTypeParameterMappings(map, callLike, signatureDecl) {
		...
		const typeArgs = this.checker.getTypeArguments(instanceType);
		...
		for (let i = 0; i < Math.min(typeParams.length, typeArgs.length); i++) {
			...
			const symbol = this.checker.getSymbolAtLocation(typeParam.name);
			if (!symbol) continue;
			const typeArgNode = this.checker.typeToTypeNode(typeArgs[i], signatureDecl, this.ts.NodeBuilderFlags.NoTruncation);
	...
	inferTypeArgumentFromParameters(callLike, signatureDecl, typeParamSymbol) {
		...
		for (let i = 0; i < Math.min(args.length, params.length); i++) {
			...
			if (this.getTypeReferenceSymbol(param.type) !== typeParamSymbol) continue;
			const inferredType = this.createTypeNodeFromExpression(args[i]);
			if (inferredType) return inferredType;
		}
	}
	normalizeTypeArgument(typeParam, typeArg) {
		...
		return this.checker.isTypeAssignableTo(typeArgType, constraintType) ? typeArg : typeParam.constraint;
	}
	...
	createTypeNodeFromExpression(expr) {
		...
		const ts = this.ts;
		if (isStringLikeExpression(ts, resolvedExpr)) return ts.factory.createLiteralTypeNode(ts.factory.createStringLiteral(resolvedExpr.text));
		if (ts.isNumericLiteral(resolvedExpr)) return ts.factory.createLiteralTypeNode(ts.factory.createNumericLiteral(resolvedExpr.text));
		if (ts.isBigIntLiteral(resolvedExpr)) return ts.factory.createLiteralTypeNode(ts.factory.createBigIntLiteral(resolvedExpr.text));
		if (resolvedExpr.kind === ts.SyntaxKind.TrueKeyword) return ts.factory.createLiteralTypeNode(ts.factory.createTrue());
		if (resolvedExpr.kind === ts.SyntaxKind.FalseKeyword) return ts.factory.createLiteralTypeNode(ts.factory.createFalse());
		if (resolvedExpr.kind === ts.SyntaxKind.NullKeyword) return ts.factory.createLiteralTypeNode(ts.factory.createNull());
	}
	getUnionVariableInfo(symbol) {
		const decl = symbol.valueDeclaration;
		if (!decl || !(this.ts.isVariableDeclaration(decl) || this.ts.isPropertyDeclaration(decl))) return null;
	...
	createUnionInfo(type, name, initNode, entries, value) {
		...
		return new UnionInfo(type, name, initNode, entries, value, metadata.docComment, metadata.tags);
	}
	...
	getInitNode(fileName, position) {
		...
		return this.findNodeAtPos(source, position) ?? null;
	}
	getSourceFile(fileName) {
		const program = this.getProgram();
		if (!program) return null;
		if (!this.sourceFileCache.has(fileName)) this.sourceFileCache.set(fileName, program.getSourceFile(fileName) ?? null);
	...
	collectUnionMemberNodes(node, callParent, typeArgMap) {
		...
		if (ts.isUnionTypeNode(node) || ts.isIntersectionTypeNode(node) || ts.isHeritageClause(node)) return node.types.flatMap((tn) => this.collectUnionMemberNodes(tn, callParent, typeArgMap));
		if (ts.isConditionalTypeNode(node)) return this.collectConditionalTypeNode(node, typeArgMap);
		if (ts.isIndexedAccessTypeNode(node)) return this.collectIndexedAccessTypeNode(node, typeArgMap);
		if (ts.isTypeLiteralNode(node)) return this.collectTypeLiteralNode(node, typeArgMap);
		if (ts.isMappedTypeNode(node)) return this.collectMappedTypeNode(node, typeArgMap);
		if (ts.isTypeReferenceNode(node)) return this.collectTypeReferenceNode(node, typeArgMap);
		if (ts.isTypeOperatorNode(node) && node.operator === ts.SyntaxKind.KeyOfKeyword) return this.collectKeyOfKeywordTypeOperatorNode(node, callParent, typeArgMap);
		if (ts.isParenthesizedTypeNode(node)) return this.collectUnionMemberNodes(node.type, node, typeArgMap);
		if (ts.isArrayTypeNode(node)) return this.collectUnionMemberNodes(node.elementType, node, typeArgMap);
		if (ts.isTupleTypeNode(node)) return this.collectTupleTypeNode(node, typeArgMap);
		if (ts.isTypeQueryNode(node)) return this.collectTypeQueryNode(node, typeArgMap);
		if (ts.isTemplateLiteralTypeNode(node)) return this.buildTemplateLiteralNode(node, typeArgMap);
		if (ts.isLiteralTypeNode(node) || ts.isTypeNode(node)) return [calledNode(node, callParent)];
		console.warn("Unknown node type: ", node);
		return [];
	}
	collectConditionalTypeNode(node, typeArgMap) {
		return [...this.collectUnionMemberNodes(node.trueType, node, typeArgMap), ...this.collectUnionMemberNodes(node.falseType, node, typeArgMap)];
	}
	collectIndexedAccessTypeNode(node, typeArgMap) {
		...
		if (indexValues.size > 0) {
			...
			const resolvedMembers = [...indexValues].flatMap((indexValue) => this.getIndexedAccessMembersForKey(objectType, indexValue, node, typeArgMap));
			if (resolvedMembers.length > 0) return resolvedMembers;
		}
		return [...this.collectUnionMemberNodes(resolvedObjectType, node, typeArgMap), ...this.collectUnionMemberNodes(node.indexType, node, typeArgMap)];
	}
	collectTypeLiteralNode(node, typeArgMap) {
		return node.members.flatMap((member) => member.type ? this.collectUnionMemberNodes(member.type, node, typeArgMap) : []);
	}
	collectMappedTypeNode(node, typeArgMap) {
		const results = [];
		if (node.typeParameter.constraint) results.push(...this.collectUnionMemberNodes(node.typeParameter.constraint, node, typeArgMap));
		if (node.type) results.push(...this.collectUnionMemberNodes(node.type, node, typeArgMap));
		return results;
	}
	...
	getIndexedAccessMembersForKey(objectType, key, callParent, typeArgMap) {
		return (objectType.getProperty(key)?.getDeclarations() ?? []).flatMap((decl) => {
			if ((this.ts.isPropertySignature(decl) || this.ts.isPropertyDeclaration(decl)) && decl.type) return this.collectUnionMemberNodes(decl.type, callParent, typeArgMap);
			if (this.ts.isTypeAliasDeclaration(decl)) return this.collectUnionMemberNodes(decl.type, callParent, typeArgMap);
			return [];
		});
	}
	collectTypeReferenceNode(node, typeArgMap) {
		...
		if (ts.isTypeParameterDeclaration(decl)) {
			...
			if (decl.constraint) return this.collectUnionMemberNodes(decl.constraint, node, typeArgMap);
			return [];
		}
	...
	collectKeyOfKeywordTypeOperatorNode(node, callParent, typeArgMap) {
		const ts = this.ts, checker = this.checker, targetTypeNode = this.resolveMappedTypeNode(node.type, typeArgMap) ?? node.type;
		return checker.getTypeAtLocation(targetTypeNode).getProperties().map((prop) => {
			const decl = prop.getDeclarations()?.[0];
			return calledNode(ts.factory.createLiteralTypeNode(ts.factory.createStringLiteral(prop.getName())), callParent, decl);
		});
	}
	collectTupleTypeNode(node, typeArgMap) {
		return node.elements.flatMap((element) => this.collectUnionMemberNodes(element, node, typeArgMap));
	}
	collectTypeQueryNode(node, typeArgMap) {
		const symbol = this.checker.getSymbolAtLocation(node.exprName);
		if (!symbol) return [];
		return (symbol.getDeclarations() ?? []).flatMap((decl) => this.collectUnionMemberNodes(decl, node, typeArgMap));
	}
	createLiteralNode(node, text, callParent, isRegexPattern) {
		const litNode = this.ts.factory.createStringLiteral(text);
		const called = node;
		const original = (called.isRegexPattern === true && called.callParent != null && called.callParent !== node && this.ts.isTemplateLiteralTypeNode(called.callParent) ? called.callParent : void 0) ?? called.original ?? node;
	...
	buildTemplateLiteralNode(node, typeArgMap) {
		...
		for (const span of node.templateSpans) {
			const spanNodes = [];
			const innerTypeNodes = this.collectUnionMemberNodes(span.type, node, typeArgMap);
			for (const typeNode of innerTypeNodes) if (typeNode.isRegexPattern != null) {
		...
		return cartesianProduct(nodes).flatMap((componentNodes) => {
			const isRegex = componentNodes.some((entry) => entry.isRegexPattern === true);
			const getText = (entry) => isRegex && entry.isRegexPattern === false ? escapeRegExp(entry.text) : entry.text;
			const fullText = (isRegex ? escapeRegExp(headText) : headText) + componentNodes.map(getText).join("");
			return componentNodes.map((entry) => this.createLiteralNode(entry, fullText, node, isRegex));
		});
	}
	...
	cmp(expr, node) {
		const resolvedExpr = this.resolveExpression(expr);
		if (isRegexNode(node) && isStringLikeExpression(this.ts, resolvedExpr)) return new RegExp(`^${node.text}$`).test(resolvedExpr.text);
	...
	cmpLit(expr, typeLiteral) {
		const ts = this.ts;
		if (isStringLikeExpression(ts, expr) && ts.isStringLiteral(typeLiteral)) return expr.text === typeLiteral.text;
		if (ts.isNumericLiteral(expr) && ts.isNumericLiteral(typeLiteral)) return expr.text === typeLiteral.text;
		if (ts.isBigIntLiteral(expr) && ts.isBigIntLiteral(typeLiteral)) return expr.text === typeLiteral.text;
		if (expr.kind === ts.SyntaxKind.TrueKeyword && typeLiteral.kind === ts.SyntaxKind.TrueKeyword || expr.kind === ts.SyntaxKind.FalseKeyword && typeLiteral.kind === ts.SyntaxKind.FalseKeyword) return true;
		if (expr.kind === ts.SyntaxKind.NullKeyword && typeLiteral.kind === ts.SyntaxKind.NullKeyword) return true;
		if (expr.kind === ts.SyntaxKind.UndefinedKeyword && typeLiteral.kind === ts.SyntaxKind.UndefinedKeyword) return true;
		return false;
	}
	...
	getReferencedSymbol(expr) {
		if (!this.ts.isIdentifier(expr) && !this.ts.isPropertyAccessExpression(expr)) return null;
	...
	getCompletionEntryName(node) {
		...
		if (this.ts.isStringLiteral(literal) || this.ts.isNumericLiteral(literal) || this.ts.isBigIntLiteral(literal)) return literal.text;
...
function extractJSDocMetadataFromNode(ts, node) {
	...
	const comment = getLeadingComment(ts, sourceText, sourceNode.getStart());
	return comment ? prepareJSDocMetadata(sourceText.substring(comment.pos, comment.end)) : {
		docComment: [],
		tags: []
	};
}
function getLeadingComment(ts, text, pos) {
	...
	if (comments.length > 0 && text[comments[0].pos + 2] === "*") return comments[comments.length - 1];
...
function extractJSDocTags(normalizedLines) {
	const tags = [];
	for (let i = 0; i < normalizedLines.length; i++) {
		const match = normalizedLines[i].match(/^@(\w+)\b(?:\s+(.*))?$/);
		if (!match) continue;
...
function cartesianProduct(arrays) {
	return arrays.reduce((acc, curr) => acc.flatMap((items) => curr.map((item) => [...items, item])), [[]]);
}
//#endregion
...
function addDeprecatedCompletionEntryDetails(details, unionInfo) {
	...
	if (details.tags.some((tag) => tag.name === deprecatedTag.name && getTagText(tag) === getTagText(deprecatedTag))) return;
...
function markDeprecatedEntries(completion, entryInfos) {
	const deprecatedNames = new Set(entryInfos.filter((entryInfo) => getDeprecatedTag(entryInfo.tags)).map((entryInfo) => entryInfo.name));
...
function appendKindModifier(kindModifiers, modifier) {
	const modifiers = new Set((kindModifiers ?? "").split(",").map((part) => part.trim()).filter(Boolean));
...
function addExtraQuickInfo(_ts, quickInfo, typesInfo) {
	...
	switch (primaryInfo.type) {
		case SupportedType.Parameter: return addExtraParamTagInfo(quickInfo, typesInfo);
		case SupportedType.Variable: return addExtraVariableQuickInfo(quickInfo, typesInfo);
	}
}
...
function buildParamTags(typesInfo, existingParamTags) {
	...
	for (const typeInfo of typesInfo) {
		...
		const tag = findJsDocParamTagByName(existingParamTags, typeInfo.name)?.tag ?? defaultParamJSDocTag(typeInfo.name);
		paramTags.push(addParamTagDescription(tag, typeInfo.docComment, typeInfo.tags));
	}
	return paramTags;
}
...
function findJsDocParamTagByName(tags, name) {
	return tags.find(({ tag }) => tag.text?.some((textPart) => textPart.kind === "parameterName" && textPart.text.toLowerCase() === name.toLowerCase())) ?? null;
}
...
function formatQuotedParamDocComment(lines, extraTags) {
	const parts = [];
	if (lines && lines.length > 0) parts.push(...quoteLines(lines));
...
function formatQuotedParamTag(tag) {
	...
	if (isMarkdownTableLine(firstLine)) return [
		"> _@" + tag.name + "_",
		"> ",
		...lines.map((line) => `> ${line}`)
	];
	...
	create(info) {
		...
		proxy.getCompletionsAtPosition = (fileName, pos, opts, fmt) => {
			...
			try {
				...
				if (completionInfo) applyCompletionInfo(ts, completions, completionInfo);
			} catch (err) {
		...
		proxy.getCompletionEntryDetails = (fileName, position, name, formatOptions, source, preferences, data) => {
			const details = languageService.getCompletionEntryDetails(fileName, position, name, formatOptions, source, preferences, data);
			if (!details) return details;
			try {
				const entryInfo = typeInfoFactory.getCompletionEntryInfo(fileName, position, name);
		...
		proxy.getSemanticDiagnostics = (fileName) => appendDeprecatedDiagnostics(languageService.getSemanticDiagnostics(fileName), () => createDeprecatedSemanticDiagnostics(this.ts, typeInfoFactory.getDeprecatedUsageInfos(fileName)), (err) => logPluginError(this.ts, logger, "Semantic Diagnostics", err));
...
function logPluginError(ts, logger, at, err) {
	logger.msg(`[TS Union Docs ${at} Error]: ${errToString(err)}`, ts.server.Msg.Err);
}
